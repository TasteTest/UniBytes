...
public class StripeService : IStripeService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!userInfoResult.IsSuccess || userInfoResult.Value == null)
            {
                _logger.LogWarning("Failed to authenticate user: {Error}", userInfoResult.Error);
                return Result<CheckoutSessionResponse>.Failure("Authentication failed: " + userInfoResult.Error);
            }

            var userInfo = userInfoResult.Value;
            var userId = Guid.Parse(userInfo.Id);

            _logger.LogInformation("User authenticated: {UserId} ({Email})", userId, userInfo.Email);

            // Check for idempotency
            if (!string.IsNullOrEmpty(request.IdempotencyKey))
            {
                var keyExists = await _unitOfWork.IdempotencyKeys.KeyExistsAsync(request.IdempotencyKey, cancellationToken);
                if (keyExists)
                {
            ...
            if (!string.IsNullOrEmpty(request.IdempotencyKey))
            {
                ...
                await _unitOfWork.IdempotencyKeys.AddAsync(idempotencyKey, cancellationToken);
                await _unitOfWork.SaveChangesAsync(cancellationToken);
            }
    ...
    public async Task<Result<PaymentResponse>> VerifyPaymentAsync(string sessionId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var payment = await _unitOfWork.Payments.GetByProviderPaymentIdAsync(sessionId, cancellationToken);
            if (payment == null)
            {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var payment = await _unitOfWork.Payments.GetByIdAsync(paymentGuid, cancellationToken);
            if (payment == null)
            {
                return Result<PaymentResponse>.Failure("Payment not found");
            }
